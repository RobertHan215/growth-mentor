        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              // LLM auto-select: whenever no model is selected (not just true first load),
              // prefer server-configured providers so users never need to pick a model manually.
                ...
                for (const [pid, cfg] of Object.entries(newProvidersConfig)) {
                  if (cfg.isServerConfigured) {
                    ...
                    const modelId = serverModels?.length
                      ? serverModels[0]
                      : cfg.models?.[0]?.id || PROVIDERS[pid as ProviderId]?.models[0]?.id;
                    if (modelId) {
