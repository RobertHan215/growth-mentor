...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  setOutlines: (outlines) => {
    ...
    if (stageId) {
      import('@/lib/hybrid-storage').then(({ saveStageOutlines }) => {
        saveStageOutlines({
          stageId,
          outlines,
        });
  ...
  saveToStorage: async (userId?: string) => {
    ...
    try {
      ...
      await saveStageData(
        stage.id,
        {
          stage: { ...stage, userId: effectiveUserId },
          scenes,
          currentSceneId,
          chats,
        },
  ...
  loadFromStorage: async (stageId: string) => {
    try {
      ...
      if (currentState.stage?.id === stageId && currentState.scenes.length > 0) {
        log.info('Stage already loaded in memory, skipping MySQL load:', stageId);
        return;
      }
      ...
      const { getStageOutlines } = await import('@/lib/hybrid-storage');
      const outlinesRecord = await getStageOutlines(stageId);
      const outlines: SceneOutline[] = outlinesRecord?.outlines || [];
