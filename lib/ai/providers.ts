...
function getCompatThinkingBodyParams(
  ...
  if (config.enabled === false) {
    switch (providerId) {
      ...
      // Qwen uses { chat_template_kwargs: { enable_thinking: false } }
      case 'qwen':
        return { chat_template_kwargs: { enable_thinking: false } };
      // SiliconFlow uses { enable_thinking: false }
      case 'siliconflow':
...
export function getModel(config: ModelConfig): ModelWithInfo {
  ...
  switch (providerType) {
    case 'openai': {
      ...
      // Model-specific extra body params injected unconditionally, regardless of
      ...
      if (config.providerId !== 'openai' || modelExtraParams || isQwenCompatBaseUrl) {
        ...
        openaiOptions.fetch = async (url: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (init?.body && typeof init.body === 'string') {
            ...
            // Model-specific overrides take highest priority (merged last)
            if (modelExtraParams) {
