 ...
 * fetches result blobs, stores in MySQL, and updates the Zustand store.
 */
...
async function generateSingleMedia(
  ...
  try {
    ...
    // Persist non-retryable failures to MySQL so they survive page refresh
    if (errorCode) {
      await saveMediaFile({
        id: mediaFileKey(stageId, req.elementId),
        stageId,
        type: req.type,
        blob: new Blob([], { type: req.type === 'image' ? 'image/png' : 'video/mp4' }),
