...
GrowthMentor generation does not automatically reuse the OpenClaw agent's current model or API key.

GrowthMentor server APIs resolve their own model and provider keys from GrowthMentor server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit GrowthMentor server-side config files.
...
- GrowthMentor server fallback is currently `gpt-4o-mini` if `DEFAULT_MODEL` is unset.
...
Do not recommend bare model IDs such as `gemini-3-flash-preview` by themselves, because GrowthMentor will otherwise parse them as OpenAI models.
...
If using a non-default provider for practice generation, also set the model selection explicitly:
...
- "I recommend configuring GrowthMentor through `.env.local` first. Please edit that file locally and tell me when you're done."
...
These are all optional. The practice generation works without them — they only unlock richer content.
